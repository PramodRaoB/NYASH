...
int parse_command(vector *tokens) {
    ...
    if (inputFd < 0 || outputFd < 0) {
        fprintf(stderr, RED "NYASH: Error with file descriptor\n" RESET);
        return 1;
    }
    ...
    for (int i = 0; i < tokens->size; i++) {
        if (strcmp(tokens->arr[i], "<") == 0) {
            if (i == tokens->size - 1) {
                fprintf(stderr, RED "NYASH: Error invalid input file\n" RESET);
                reset_io(inputFd, outputFd);
                return 1;
            }
            ...
            if (i == tokens->size - 1) {
                fprintf(stderr, RED "NYASH: Error invalid output file\n" RESET);
                reset_io(inputFd, outputFd);
                return 1;
            }
